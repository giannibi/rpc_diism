...
def mpc_controller_regulation(parameters, T, **kwargs):
        '''
...
def mpc_controller_staticref(parameters, T, **kwargs):
        """
        Create an I/O system implementing an MPC controller
        parameters: Controller parameters
...
def terminal_cost_set_with_relax(p):
    ...
    if problem.status == 'unbounded':
        warnings.warn("WARNING: terminal set is unbounded: is your problem even constrained?",
               "If not, use LQR :) ")
        Qt = np.zeros([n,n])
        ...
        warnings.warn("WARNING: terminal set is empty: probably the origin",
...
def mpc_controller_with_relax(parameters, T, **kwargs):
    ...
    if p.Xf == 'lqr':
        ...
        # Terminal set is unbounded: Qt=0
            ...
            warnings.warn("Terminal set is empty: falling back to zero terminal", 
    ...
    def update_mpc_problem(ref, t):
        ...
        if status == 'unbounded':
            ...
            warnings.warn(f"Relaxation triggered for empty terminal set at t={t}")
    ...
    def _update(t, x, u, params={}):
        ...
        if problem.status != 'optimal':
            ...
            if p.alpharelax is not None:
                ...
                warnings.warn(f"Relaxation triggered for infeasibility at t={t}")
                p.alpharelax = p.alpharelaxe
                p.P, p.Qt, status = terminal_cost_set_with_relax(p)
                problem = quadratic_mpc_problem_with_relax(p)
                # Solve again 
